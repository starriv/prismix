...
export default function RequestVolumeChart({
  ...
  data: { date: string; requests: number; totalTokens: number }[];
}) {
    ...
    <Card className="py-0">
      <CardHeader className="flex flex-col items-stretch border-b !p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 pt-4 pb-3 sm:!py-0">
          <CardTitle className="text-sm">{t("dash.ai.volume-title")}</CardTitle>
          <CardDescription className="text-xs">{t("dash.ai.volume-desc")}</CardDescription>
        </div>
        <div className="flex">
          {(["requests", "tokens"] as const).map((key) => (
            ...
            >
              <span className="text-xs text-muted-foreground">{t(`dash.ai.volume-${key}`)}</span>
