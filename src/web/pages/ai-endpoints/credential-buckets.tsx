...
export function EndpointCredentialBucketsSection({ endpoint }: { endpoint: AiEndpoint }) {
  ...
  const buckets = useMemo<CredentialBucket[]>(() => {
    ...
    const officialBucket: CredentialBucket = {
      id: "official",
      name: t("ai-endpoints.credentials.official-bucket"),
      kind: null,
      upstreamId: null,
  ...
  const handleToggle = useCallback(
    async (credential: AiEndpointCredential) => {
      try {
        ...
        toast.success(t("ai-endpoints.credentials.toast.updated"));
      } catch {
        toast.error(t("ai-endpoints.credentials.toast.update-error"));
      }
    },
    [updateCredential, t],
  );
  ...
  const handleTest = useCallback(
    async (credential: AiEndpointCredential) => {
      try {
        ...
        if (result.success) {
          toast.success(t("ai-endpoints.credentials.toast.test-ok", { ms: result.latencyMs ?? 0 }));
        } else {
          toast.error(result.error ?? t("ai-endpoints.credentials.toast.test-error"));
        }
      } catch (err) {
        toast.error(
          err instanceof Error ? err.message : t("ai-endpoints.credentials.toast.test-error"),
        );
      }
    },
    [testCredential, t],
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t("ai-endpoints.credentials.toast.deleted"));
      setDeleteTarget(null);
    } catch {
      toast.error(t("ai-endpoints.credentials.toast.delete-error"));
    }
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <CardTitle className="text-sm">
                {t("ai-endpoints.credentials.section-title")}
              </CardTitle>
              <p className="text-sm text-muted-foreground">{t("ai-endpoints.credentials.desc")}</p>
            </div>
                ...
                <Select value={strategy} onValueChange={handleStrategyChange}>
                  ...
                  <SelectContent>
                    <SelectItem value="round-robin">
                      {t("ai-endpoints.credentials.strategy.round-robin")}
                    </SelectItem>
                    <SelectItem value="random">
                      {t("ai-endpoints.credentials.strategy.random")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              )}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("ai-endpoints.credentials.dialog.delete-title")}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <p className="text-sm text-muted-foreground">
              {t("ai-endpoints.credentials.dialog.delete-body", { name: deleteTarget?.name ?? "" })}
            </p>
          </DialogBody>
          <DialogFooter>
    ...
    <div className={cn("rounded-lg border", !bucket.enabled && "opacity-60")}>
      <div className="flex items-center justify-between gap-3 px-3 py-2.5 border-b bg-muted/30">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {isOfficial && (
            <Tooltip>
              ...
              <TooltipContent>{t("ai-endpoints.credentials.official-hint")}</TooltipContent>
            </Tooltip>
          )}
          {!isOfficial && (
          ...
          <Badge variant="outline" className="text-[10px]">
            {t("ai-endpoints.credentials.count", {
        ...
        >
          ...
          {t("ai-endpoints.credentials.add")}
        </Button>
      </div>
      ...
      <div className="p-2 space-y-1.5">
        {bucket.credentials.length === 0 ? (
          <div className="px-3 py-4 text-center text-xs text-muted-foreground">
            {t("ai-endpoints.credentials.empty")}
          </div>
        ) : (
            ...
            return (
              ...
              >
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {!credential.enabled
                          ...
                          : isNext
                            ? t("ai-endpoints.credentials.next")
                    ...
                    {showPool && (
                      <div className="flex items-center gap-0.5 mr-1">
                        <Button
                          ...
                          disabled={weight <= 0}
                          aria-label={t("ai-endpoints.credentials.weight-down")}
                        >
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            {t("ai-endpoints.credentials.weight-hint")}
                          </TooltipContent>
                        </Tooltip>
                        <Button
                          variant="ghost"
                          ...
                          onClick={() => onWeightChange(credential, 1)}
                          disabled={weight >= 100}
                          aria-label={t("ai-endpoints.credentials.weight-up")}
                        >
                          <span className="text-xs font-bold">+</span>
                        </Button>
                      </div>
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-3">
                  ...
                  >
                    {credential.ownerName ?? t("ai-endpoints.credentials.tag.platform")}
                  </Badge>
                  <span className="text-xs text-muted-foreground tabular-nums">
                    {credential.lastUsedAt
                      ? formatDistanceToNow(new Date(credential.lastUsedAt), { addSuffix: true })
                      : t("ai-endpoints.credentials.never")}
                  </span>
                </div>
              </div>
            );
...
function AddCredentialToBucketDialog({
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    if (!bucket || !bucket.enabled) {
      toast.error(t("ai-endpoints.credentials.toast.create-error"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("ai-endpoints.credentials.toast.created"));
      onOpenChange(false);
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : t("ai-endpoints.credentials.toast.create-error"),
      );
    }
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent preventClose>
        <DialogHeader>
          <DialogTitle>
            {t("ai-endpoints.credentials.dialog.add-title", { upstream: bucket?.name ?? "" })}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={handleSubmit}>
            <DialogBody className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("ai-endpoints.credentials.form.name")}</FormLabel>
                    <FormControl>
                      <Input placeholder={t("ai-endpoints.credentials.form.name-ph")} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {isCloudflare
                        ? t("ai-endpoints.credentials.form.cloudflare-client-secret")
                        : isSigV4
                          ? t("ai-endpoints.credentials.form.secret-access-key")
                          : t("ai-endpoints.credentials.form.api-key")}
                    </FormLabel>
                    <FormControl>
                      <SecretInput
                        placeholder={
                          isCloudflare
                            ? t("ai-endpoints.credentials.form.cloudflare-client-secret-ph")
                            : isSigV4
                              ? t("ai-endpoints.credentials.form.secret-access-key-ph")
                              : t("ai-endpoints.credentials.form.api-key-ph")
                        }
                        {...field}
                      />
                    </FormControl>
                    <p className="text-[11px] text-muted-foreground">
                      {isCloudflare
                        ? t("ai-endpoints.credentials.form.cloudflare-client-secret-hint")
                        : isSigV4
                          ? t("ai-endpoints.credentials.form.secret-access-key-hint")
                          : t("ai-endpoints.credentials.form.api-key-hint")}
                    </p>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              {activeKeyProviders.length > 0 && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("ai-endpoints.credentials.form.owner")}</FormLabel>
                      <Select
                        value={field.value ? String(field.value) : "none"}
                        onValueChange={(v) => field.onChange(v === "none" ? null : Number(v))}
                      >
                        <SelectTrigger className="w-full">
                          <SelectValue placeholder={t("ai-endpoints.credentials.form.owner-ph")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="none">
                            {t("ai-endpoints.credentials.form.owner-none")}
                          </SelectItem>
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {t("ai-endpoints.credentials.form.owner-hint")}
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
