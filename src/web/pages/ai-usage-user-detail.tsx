...
import { useAdminUserDetail } from "@/web/api/admin-hooks";
import {
  useAiRequestLog,
  useAiUsageByKeyForUser,
...
export default function AiUsageUserDetailPage() {
  ...
  const { data: recent = [], isLoading: recentLoading } = useAiUsageRecentByUser(
    userId,
    LIVE_REFETCH_MS,
  );
  const { data: daily = [], isLoading: dailyLoading } = useAiUsageDailyByUser(userId);
  const { data: byKeyData = [], isLoading: byKeyLoading } = useAiUsageByKeyForUser(userId);
  ...
  return (
    <div>
      <Header
        title={t("ai-usage.user-detail.title", { name: userName })}
