...
} from "./supplier-connections/constants";
import { EndpointCredentialBucketsSection } from "./supplier-connections/credential-buckets";
...
const endpointFormSchema = z
  .object({
    ...
    enabled: z.boolean(),
    concurrencyMode: z.enum(["inherit", "override"]),
    officialConcurrencyLimit: z.number().int().positive().nullable(),
  ...
  .refine((d) => d.authMode !== "override" || d.apiFormat !== "bedrock" || !!d.sigv4Region, {
    message: "common.valid.required",
    path: ["sigv4Region"],
  })
  .refine((d) => d.authMode !== "override" || d.authType !== "sigv4" || !!d.sigv4AccessKeyId, {
    message: "common.valid.required",
    path: ["sigv4AccessKeyId"],
  })
  .refine(
    (d) =>
      d.authMode !== "override" || d.authType !== "cloudflare" || !!d.cloudflareClientId?.trim(),
    {
      message: "common.valid.required",
...
function endpointAuthType(value?: string): EndpointFormValues["authType"] {
  if (value === "api-key" || value === "cloudflare" || value === "sigv4") return value;
  return "bearer";
}

function supplierEndpointDefaults(supplier?: AiSupplier) {
  const authConfig = (supplier?.authConfig ?? {}) as Record<string, unknown>;
  return {
    authType: endpointAuthType(supplier?.authType),
    apiKeyHeaderName: (authConfig.headerName as string | undefined) ?? "",
    sigv4Region: (authConfig.region as string | undefined) ?? "",
...
function endpointEffectiveRuntime(endpoint: AiEndpoint) {
  return (
    endpoint.effectiveRuntimeConfig ?? {
      authMode: endpoint.authMode,
      authType: endpoint.authType,
      authConfig: endpoint.authConfig,
      concurrencyMode: endpoint.concurrencyMode,
      officialConcurrencyLimit: endpoint.officialConcurrencyLimit ?? null,
      officialQueueTimeoutMs: endpoint.officialQueueTimeoutMs ?? 30_000,
    }
  );
}
...
export default function SupplierConnectionsPage() {
  ...
  return (
    <div>
      <Header
        title={t("supplier-connections.title")}
        description={t("supplier-connections.desc")}
      />
          ...
          <>
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => setAddOpen(true)}>
                ...
                {t("supplier-connections.btn.new")}
              </Button>
            </div>
            <EndpointGrid
              endpoints={endpoints}
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">{t("supplier-connections.empty")}</p>
        </CardContent>
      </Card>
    );
  }
...
function EndpointCard({
  ...
  const upstreamLabel =
    upstreamCount > 0
      ? t("supplier-connections.card.upstreams", { count: upstreamCount })
      : t("supplier-connections.card.no-upstreams");
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-label={t("supplier-connections.card.open-endpoint", { name: endpoint.name })}
    >
      <Card
        className={cn(
        ...
        <CardContent className="space-y-3 pb-4 pt-0">
          <div className="flex flex-wrap gap-1.5">
            ...
            <Badge variant="secondary" className="text-xs">
              {t(`supplier-connections.mode.${endpoint.authMode}`)}
            </Badge>
          </div>
...
function EndpointDetail({
  ...
  const handleConfirmDelete = useCallback(async () => {
    try {
      ...
      toast.success(t("supplier-connections.toast.deleted"));
      onBack();
    } catch {
      toast.error(t("supplier-connections.toast.delete-error"));
    }
  ...
  return (
    <>
          ...
          <div className="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-4">
            <div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t("supplier-connections.form.base-url")}
              </div>
              <div className="font-mono text-xs break-all">{endpoint.baseUrl}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t("supplier-connections.th.api-format")}
              </div>
              <div>{endpoint.apiFormat}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t("supplier-connections.th.auth-type")}
              </div>
              <div className="flex flex-wrap items-center gap-1.5">
                <span>{runtime.authType}</span>
            ...
            <div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t("supplier-connections.th.routing")}
              </div>
              <div>
                {endpoint.upstreamRoutingStrategy === "weighted-random"
                  ? t("supplier-connections.strategy.weighted-random")
                  : t("supplier-connections.strategy.priority")}
              </div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t("supplier-connections.form.concurrency-mode")}
              </div>
              <div>
                <Badge variant="secondary" className="text-[10px]">
                  {t(`supplier-connections.mode.${endpoint.concurrencyMode}`)}
                </Badge>
              </div>
            </div>
          </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("supplier-connections.dialog.delete-title")}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <p className="text-sm text-muted-foreground">
              {t("supplier-connections.dialog.delete-body", { name: endpoint.name })}
            </p>
          </DialogBody>
          <DialogFooter>
...
function EndpointUpstreamsSection({ endpoint }: { endpoint: AiEndpoint }) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t("supplier-connections.toast.upstream-deleted"));
      setDeleteTarget(null);
    } catch (err: unknown) {
      toast.error(
        err instanceof Error ? err.message : t("supplier-connections.toast.upstream-delete-error"),
      );
    }
    ...
    () => [
      {
        ...
        header: t("supplier-connections.upstreams.th.name"),
      ...
      },
      {
        ...
        header: t("supplier-connections.upstreams.th.upstream-id"),
      ...
      },
      {
        ...
        header: t("supplier-connections.upstreams.th.kind"),
      ...
      },
      {
        ...
        header: t("supplier-connections.upstreams.th.base-url"),
        meta: { headerClassName: "w-[20%]" },
      },
      {
        accessorKey: "priority",
        cell: ({ row }) => <DataTableText>{row.original.priority}</DataTableText>,
        header: t("supplier-connections.upstreams.th.priority"),
        meta: { headerClassName: "w-[8%]" },
      },
      {
        accessorKey: "weight",
        cell: ({ row }) => <DataTableText>{row.original.weight}</DataTableText>,
        header: t("supplier-connections.upstreams.th.weight"),
        meta: { headerClassName: "w-[8%]" },
      },
      {
        accessorKey: "enabled",
          ...
          <Switch
            ...
            onCheckedChange={(enabled) => {
              void updateAssignment
                ...
                .then(() => toast.success(t("supplier-connections.toast.upstream-updated")))
                .catch((err: unknown) =>
                  toast.error(
                    err instanceof Error
                      ? err.message
                      : t("supplier-connections.toast.upstream-update-error"),
                  ),
                );
            }}
          />
        ),
        header: t("supplier-connections.upstreams.th.enabled"),
        meta: { headerClassName: "w-[8%]" },
      },
      {
        id: "actions",
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <CardTitle className="text-sm">
                {t("supplier-connections.upstreams.section-title")}
              </CardTitle>
              <p className="text-sm text-muted-foreground">
                {t("supplier-connections.upstreams.desc")}
              </p>
            </div>
            <Button size="sm" onClick={() => setAssignOpen(true)}>
              <Plus className="mr-1 h-4 w-4" />
              {t("supplier-connections.btn.assign-upstream")}
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {(sortedAssignments.length > 0 || isLoading) && (
            <DataTable
              ...
              emptyText={t("supplier-connections.upstreams.empty")}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("supplier-connections.dialog.delete-upstream-title")}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <p className="text-sm text-muted-foreground">
              {t("supplier-connections.dialog.delete-upstream-body", {
  ...
  return (
    ...
    >
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-sm font-medium">
                {t("supplier-connections.upstreams.official-title")}
              </h3>
              <Badge variant="secondary" className="text-[10px]">
                {t("supplier-connections.upstreams.kind.official")}
              </Badge>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {hasAssignments
                ? t("supplier-connections.upstreams.official-fallback-desc")
                : t("supplier-connections.upstreams.official-default-desc")}
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-[minmax(0,1.4fr)_140px_120px_120px_120px]">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
            {t("supplier-connections.upstreams.th.base-url")}
          </div>
          <CopyableText
        ...
        <div>
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
            {t("supplier-connections.upstreams.route-role")}
          </div>
          <div>
            {hasAssignments
              ? t("supplier-connections.upstreams.fallback")
              : t("supplier-connections.upstreams.default-route")}
          </div>
        </div>
        <div>
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
            {t("supplier-connections.upstreams.th.upstream-id")}
          </div>
          <div className="font-mono text-xs">{t("supplier-connections.upstreams.official-id")}</div>
        </div>
        <div>
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
            {t("supplier-connections.upstreams.th.concurrency-limit")}
          </div>
          <div className="font-mono text-xs">
            {runtime.officialConcurrencyLimit ?? t("supplier-connections.upstreams.unlimited")}
          </div>
        </div>
        <div>
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
            {t("supplier-connections.upstreams.th.queue-timeout")}
          </div>
          <div className="font-mono text-xs">{runtime.officialQueueTimeoutMs ?? 30_000}ms</div>
        </div>
      </div>
    </div>
  );
...
function EndpointFormDialog({
  ...
  const watchedAuthMode = useWatch({ control: form.control, name: "authMode" });
  ...
  useEffect(() => {
    ...
    const supplier = suppliers.find((item) => item.id === watchedSupplierId);
    if (!supplier) return;
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    ...
    } else if (authMode === "override" && data.authType === "api-key") {
      authConfig = apiKeyHeaderName?.trim() ? { headerName: apiKeyHeaderName.trim() } : {};
    } else if (authMode === "override" && data.authType === "sigv4" && sigv4Region) {
      authConfig = {
        region: sigv4Region,
        service: "bedrock",
        ...(sigv4AccessKeyId ? { accessKeyId: sigv4AccessKeyId } : {}),
      };
    } else if (authMode === "override" && data.apiFormat === "bedrock" && sigv4Region) {
    ...
    try {
      if (isEdit) {
        ...
        toast.success(t("supplier-connections.toast.created"));
      }
      onOpenChange(false);
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : t("supplier-connections.toast.create-error"),
      );
    }
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent preventClose>
        <DialogHeader>
          <DialogTitle>
            {isEdit
              ? t("supplier-connections.dialog.edit-title")
              : t("supplier-connections.dialog.add-title")}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={handleSubmit}>
            <DialogBody className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("supplier-connections.form.supplier")}</FormLabel>
                    <Select
                      value={field.value ? String(field.value) : ""}
                      onValueChange={(value) => field.onChange(Number(value))}
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t("supplier-connections.form.supplier-ph")} />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("supplier-connections.form.endpoint-id")}</FormLabel>
                    <FormControl>
                      <Input
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("supplier-connections.form.name")}</FormLabel>
                    <FormControl>
                      <Input placeholder={t("supplier-connections.form.name-ph")} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("supplier-connections.form.base-url")}</FormLabel>
                    <FormControl>
                      <Input placeholder={t("supplier-connections.form.base-url-ph")} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <FormLabel>{t("supplier-connections.form.auth-mode")}</FormLabel>
                      <p className="text-[11px] text-muted-foreground">
                        {field.value === "inherit"
                          ? t("supplier-connections.form.auth-mode-inherit-hint")
                          : t("supplier-connections.form.auth-mode-override-hint")}
                      </p>
                    </div>
                    <FormControl>
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          field.onChange(checked ? "override" : "inherit")
                        }
                      />
                    </FormControl>
                  </FormItem>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.api-format")}</FormLabel>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.auth-type")}</FormLabel>
                      <Select
                        value={field.value}
                        onValueChange={field.onChange}
                        disabled={watchedAuthMode === "inherit"}
                      >
                        <SelectTrigger className="w-full">
                          <SelectValue />
                        </SelectTrigger>
              ...
              {watchedAuthType === "api-key" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.api-key-header-name")}</FormLabel>
                      <FormControl>
                        <Input
                          className="font-mono"
                          placeholder={t("supplier-connections.form.api-key-header-name-ph")}
                          disabled={watchedAuthMode === "inherit"}
                          {...field}
                        />
                      </FormControl>
                      <p className="text-[11px] text-muted-foreground">
                        {t("supplier-connections.form.api-key-header-name-hint")}
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              {watchedApiFormat === "bedrock" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.sigv4-region")}</FormLabel>
                      <Select
                        value={field.value ?? ""}
                        onValueChange={field.onChange}
                        disabled={watchedAuthMode === "inherit"}
                      >
                        <SelectTrigger className="w-full">
                          <SelectValue
                            placeholder={t("supplier-connections.form.sigv4-region-ph")}
                          />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {t("supplier-connections.form.sigv4-region-hint")}
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              {watchedAuthType === "cloudflare" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.cloudflare-client-id")}</FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t("supplier-connections.form.cloudflare-client-id-ph")}
                          className="font-mono"
                          disabled={watchedAuthMode === "inherit"}
                          {...field}
                        />
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {t("supplier-connections.form.cloudflare-client-id-hint")}
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              {watchedAuthType === "sigv4" && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.sigv4-access-key-id")}</FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t("supplier-connections.form.sigv4-access-key-id-ph")}
                          disabled={watchedAuthMode === "inherit"}
                          {...field}
                        />
                      </FormControl>
                      <p className="text-[11px] text-muted-foreground">
                        {t("supplier-connections.form.sigv4-access-key-id-hint")}
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <FormLabel>{t("supplier-connections.form.concurrency-mode")}</FormLabel>
                      <p className="text-[11px] text-muted-foreground">
                        {field.value === "inherit"
                          ? t("supplier-connections.form.concurrency-mode-inherit-hint")
                          : t("supplier-connections.form.concurrency-mode-override-hint")}
                      </p>
                    </div>
                    <FormControl>
                      <Switch
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("supplier-connections.form.official-concurrency-limit")}
                      </FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={1}
                          placeholder={t("supplier-connections.form.official-concurrency-limit-ph")}
                          value={field.value ?? ""}
                          onChange={(event) =>
                            field.onChange(
                          ...
                          name={field.name}
                          ref={field.ref}
                          disabled={watchedConcurrencyMode === "inherit"}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.form.official-queue-timeout")}</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={1}
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={createEndpoint.isPending || updateEndpoint.isPending}>
                {isEdit ? t("common.btn.save") : t("supplier-connections.btn.create")}
              </Button>
            </DialogFooter>
          </form>
        </Form>
...
function AssignUpstreamDialog({
  ...
  const handleSubmit = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.success(t("supplier-connections.toast.upstream-assigned"));
      onOpenChange(false);
    } catch (err: unknown) {
      toast.error(
        err instanceof Error ? err.message : t("supplier-connections.toast.upstream-assign-error"),
      );
    }
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent preventClose>
        <DialogHeader>
          <DialogTitle>{t("supplier-connections.dialog.assign-upstream-title")}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={handleSubmit}>
            <DialogBody className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("supplier-connections.upstreams.form.upstream")}</FormLabel>
                    <Select
                      value={field.value ? String(field.value) : ""}
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue
                          placeholder={t("supplier-connections.upstreams.form.upstream-ph")}
                        />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    {availableUpstreams.length === 0 && (
                      <p className="text-[11px] text-muted-foreground">
                        {t("supplier-connections.upstreams.form.no-available")}
                      </p>
                    )}
                    <FormMessage />
                  </FormItem>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.upstreams.form.priority")}</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={0}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("supplier-connections.upstreams.form.weight")}</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min={0}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <FormLabel>{t("supplier-connections.upstreams.form.enabled")}</FormLabel>
                    </div>
                    <FormControl>
...
function EditAssignmentDialog({
  ...
  const handleSubmit = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.success(t("supplier-connections.toast.upstream-updated"));
      onOpenChange(false);
    } catch (err: unknown) {
      toast.error(
        err instanceof Error ? err.message : t("supplier-connections.toast.upstream-update-error"),
      );
    }
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent preventClose>
        <DialogHeader>
          <DialogTitle>{t("supplier-connections.dialog.edit-upstream-title")}</DialogTitle>
        </DialogHeader>
        {assignment && (
