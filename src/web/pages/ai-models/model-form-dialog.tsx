...
export function ModelFormDialog({
  ...
  useEffect(() => {
    if (open && model) {
      ...
      if (discoverEnabled) fetchModels();
    }
  }, [open, model, form, fetchModels, discoverEnabled]);

  const handleToggleDiscover = useCallback(
    (checked: boolean) => {
      setDiscoverEnabled(checked);
      if (checked) fetchModels();
    },
    [fetchModels],
  );
  ...
  const hasKey = keys.some((k) => k.providerId === providerId && k.enabled);
  const noKey =
  ...
  const handleBatchCreate = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t("ai-models.toast.batch-created", { count: result.created }));
      onOpenChange(false);
    } catch (err) {
        ...
        <Form {...form}>
          <form onSubmit={isBatchMode ? (e) => e.preventDefault() : handleSubmit}>
            <DialogBody className="space-y-4">
              ...
              {/* Loading */}
              {!isEdit && discoverEnabled && discovering && (
