...
用户请求 model = "claude-opus-4-7"
   │
   ▼
第一层：模型 → 供应商        (ai_model_routes)
   决定由哪个供应商处理，可在路由级覆盖模型名
   输出: providerModelId = route.providerModelId ?? model.modelId
   │
   ▼
第二层：供应商 → 上游        (ai_upstream_assignments)
...
第三层：上游 → 实际模型名     (ai_upstream_model_mappings)
   ...
   输出: effectiveModelId = mapping[providerModelId] ?? providerModelId
   │
   ▼
POST {upstream.baseUrl}  Body: { model: effectiveModelId, ... }
```

解析优先级：`upstream_mapping > route.providerModelId > model.modelId`。
...
[`src/server/ai/lib/key-balancer.ts`](src/server/ai/lib/key-balancer.ts)
...
- 30s TTL 内存缓存，key 为 `providerId`
- 始终追加 legacy target（provider 自身 `baseUrl`，priority=1000）作为兜底
...
[`src/server/ai/lib/provider-auth.ts`](src/server/ai/lib/provider-auth.ts)
...
[`src/server/ai/providers/`](src/server/ai/providers/)
...
Anthropic 客户端协议（`/v1/messages`）在网关内被转码为 canonical OpenAI Chat 格式，可路由到任意 provider，响应再转回 Anthropic 形态。
...
通用 usage 提取通过 SSE event `type` 字段区分 OpenAI / Anthropic / Gemini / OpenAI Responses API，避免每个 provider 写独立解析器。
...
| `ai-key-touch`       | write handler           | 更新 key `last_used_at`      |
...
| `ai_model_routes`              | 模型 → 供应商路由（priority、weight、providerModelId） |
| `ai_providers`                 | 供应商配置（apiFormat、authType、负载均衡策略）        |
| `ai_keys`                      | 上游 API Key（AES-256-GCM 加密存储）                   |
| `ai_upstreams`                 | 上游端点（baseUrl、kind、modelsEndpoint）              |
| `ai_upstream_assignments`      | 供应商 ↔ 上游 N:N 绑定                                 |
| `ai_upstream_model_mappings`   | 上游级模型名重映射                                     |
| `ai_guardrail_config`          | 内容审核规则                                           |

### 关键设计决策

1. **三层路由解耦** — 模型名、供应商、上游、上游模型名四层标识独立配置，支持「同一模型多供应商」「同供应商多上游」「同上游不同模型名映射」。
2. **内存 SWRR Key 池** — 绕过 DB LRU 竞态，Nginx 风格平滑加权轮转 + 指数退避惩罚。
...
Request model = "claude-opus-4-7"
   │
   ▼
Layer 1: model → provider        (ai_model_routes)
   Decides which provider serves the model. Route can override the model name.
   Output: providerModelId = route.providerModelId ?? model.modelId
   │
   ▼
Layer 2: provider → upstream     (ai_upstream_assignments)
...
Layer 3: upstream → effective model name   (ai_upstream_model_mappings)
   ...
   Output: effectiveModelId = mapping[providerModelId] ?? providerModelId
   │
   ▼
POST {upstream.baseUrl}  Body: { model: effectiveModelId, ... }
```

Resolution precedence: `upstream_mapping > route.providerModelId > model.modelId`.
...
- 30s TTL in-memory cache keyed by `providerId`
- Always appends a legacy target (provider's own `baseUrl`, priority=1000) as fallback
...
The Anthropic client protocol (`/v1/messages`) is transcoded to a canonical OpenAI Chat format inside the gateway, routable to any provider, then converted back to Anthropic shape on response.
...
Universal usage extraction discriminates OpenAI / Anthropic / Gemini / OpenAI Responses API by SSE event `type`, avoiding per-provider parsers.
...
| `ai-key-touch`       | write handler          | Update key `last_used_at`                       |
...
| `ai_model_routes`              | Model → provider routes (priority, weight, providerModelId)  |
| `ai_providers`                 | Provider config (apiFormat, authType, load-balance strategy) |
| `ai_keys`                      | Upstream API keys (AES-256-GCM at rest)                      |
| `ai_upstreams`                 | Upstream endpoints (baseUrl, kind, modelsEndpoint)           |
| `ai_upstream_assignments`      | Provider ↔ upstream N:N binding                              |
| `ai_upstream_model_mappings`   | Per-upstream model name override                             |
| `ai_guardrail_config`          | Content moderation rules                                     |

### Key Design Decisions

1. **Three-layer routing decoupling** — Model name, provider, upstream, and per-upstream model name are independently configured, supporting "one model → many providers", "one provider → many upstreams", "one upstream → different model names".
2. **In-memory SWRR key pool** — Bypasses DB LRU races with Nginx-style smooth weighted rotation + exponential backoff penalty.
