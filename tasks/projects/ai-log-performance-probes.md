...
- 2026-06-30: Reviewed production-style cache summary ambiguity and updated aggregate fields, UI labels, docs, and tests so gateway cache hit rate shows unavailable when no `hit + miss` denominator exists. Removed premature `cacheDisabled`/`cacheObservedRequests` fields after confirming no write-site emits `cacheStatus: "disabled"`; the enum value stays reserved for a future config flag.
- 2026-06-30: Completed field credibility review after user-log detail screenshot. Removed fake first-token writes, renamed compact streaming latency to first chunk, conditionally hid non-applicable detail fields, and documented which metrics are measured versus unavailable.
