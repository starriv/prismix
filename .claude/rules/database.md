...
`drizzle/` directory is **generated output** — never hand-edit files inside it. If a migration needs supplementary steps (data backfill, de-duplication, constraint fixup), write a standalone guide in `docs/deploy/` instead of patching the generated SQL.

❌ Claude running any migration or DB reset command.
❌ Silently changing schema without the migration warning.
❌ Editing files under `drizzle/` (generated by drizzle-kit, not hand-authored).
❌ Inlining manual SQL steps into generated migration files — put them in `docs/deploy/`.
