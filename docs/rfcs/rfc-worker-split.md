...
serve({ port }) → bootstrap() → 全部启动
                                 ...
                                 │   ├── endpoint-health-check (concurrency 1)
...
- Shutdown 函数（line 184-216）：去掉所有 worker-only 的 stop/close 调用（`stopTopupExpiryJob` / `closeDepositScanQueue` / `closeEndpointHealthCheckJob` / `closeLimitedFreeModelExpiryJob` / `stopLiteLLMPricingJob` / `stopWebhookRetryJob`），因为 API 进程不再启动它们
...
| `src/server/jobs/check-endpoint-health.ts:364,385`    | `endpoint-health-check`     | 1           | BullMQ repeatable 60s  |     ✅     |
...
| `src/server/ai/lib/billing.ts:302`                         | `ai-endpoint-credential-touch` | 更新上游凭证 last_used_at      |
...
| `src/server/ai/routes/relay.ts:376,633`                    | `ai-endpoint-credential-touch` | Admin relay                    |
...
| `src/server/ai/lib/stream-proxy.ts:501,755`                | `ai-endpoint-credential-touch` | 流式代理                       |
