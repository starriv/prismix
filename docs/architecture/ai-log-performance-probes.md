...
| `firstTokenMs`    | 流式请求从 relay 开始处理到收到首个携带生成内容的 delta 的耗时。包含 Anthropic `thinking_delta`（推理 token）、OpenAI Chat 扩展 `delta.reasoning_content`（GLM/DeepSeek/Qwen/SiliconFlow 思考模型）和普通文本 delta；不包含 `message_start`、`ping`、`content_block_start` 等控制事件。对思考模型，此字段反映首个生成 token（推理或文本）的时刻，而非首个可见文本 token。                                                  |
| `tokensPerSecond` | 流式输出吞吐率，`visibleOutputTokens * 1000 / decodeMs`。`visibleOutputTokens = outputTokens - reasoningTokens`（OpenAI o 系列有 reasoningTokens 拆分时扣减；Anthropic 思考模型无拆分，分子保留全部 outputTokens，但 `firstTokenMs` 已前移到首个思考 token，分母同步覆盖思考阶段）。`decodeMs = latencyMs - firstTokenMs`。超过 `TPS_SANITY_CAP`（5000 tok/s）的值会被截断到该上限，避免思考模型小 decode 窗口导致的虚高。 |
