...
| `firstTokenMs`    | 流式请求从 relay 开始处理到收到首个携带生成内容的 delta 的耗时。包含 Anthropic `thinking_delta`（推理 token）和普通文本 delta；不包含 `message_start`、`ping`、`content_block_start` 等控制事件。对思考模型，此字段反映首个生成 token（推理或文本）的时刻，而非首个可见文本 token。                                                                                                                                        |
