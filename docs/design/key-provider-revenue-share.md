...
允许第三方（密钥合作方）向平台提供 AI API Key，平台使用这些 Key 为消费者提供服务，合作方按实际用量赚取分润收益。

## 核心思路

现有 `ai_keys` 表已经形成一个统一密钥池，`key-balancer` 按 weight 做负载均衡选 key。改动只需：
...
ai_keys 表 (混合池)
  ...
  ├── key#2  ownerId=1      weight=1  (合作方#1)
  ├── key#3  ownerId=1      weight=1  (合作方#1)
...
| 能力                | 实现方式                                                         |
| ------------------- | ---------------------------------------------------------------- |
| 合作方自助注册      | `key_providers` 关联 `users` 表，用户自行上传 key                |
| 按 model 差异化分润 | 分润公式加 model 权重系数                                        |
| 合作方 dashboard    | 复用 `ai_usage_logs` + `WHERE keyId IN (合作方的 keys)`          |
| 自动结算/提现       | 复用已有 `withdraw_orders` 流程                                  |
| Key 质量监控        | `ai_usage_logs` 已有 `statusCode` + `latencyMs`，按 `keyId` 聚合 |
| 合作方限流          | 在 key-balancer 层面根据 ownerId 做额外 rate-limit               |
