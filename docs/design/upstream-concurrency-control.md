...
Protect a configured upstream target by limiting in-flight relay requests per stable target scope. Requests that arrive above the configured limit wait briefly for a slot and then either proceed, try the next candidate, or return an upstream-exhausted error.

## Scope

- Scope key:
  - Global upstreams: numeric `ai_upstreams.id`.
  - Provider official upstreams: `provider:{provider.id}:official`.
- Official provider default URLs (`upstreamId = null`) are limited through provider-level official upstream settings.
...
1. Add upstream config fields:
   - Global upstream: `concurrencyLimit`, `queueTimeoutMs`.
   - Provider official upstream: `officialConcurrencyLimit`, `officialQueueTimeoutMs`.
   - Limit is a nullable positive integer; null means unlimited.
   - Queue timeout is a positive integer; defaults to 30 seconds.
...
4. Expose configuration in admin upstream forms/detail views and provider official upstream UI.
